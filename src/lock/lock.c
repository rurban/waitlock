...
char *find_lock_directory(void) {
  ...
  const char *candidates[] = {"/var/run/waitlock", "/run/waitlock",
                              "/var/lock/waitlock", "/tmp/waitlock", NULL};
  const char **p;
  char *home;
...
int acquire_lock(const char *descriptor, int max_holders, double timeout) {
  ...
  if (!lock_dir) {
    error(E_NODIR,
          "Cannot find or create lock directory (tried: %s, %s, %s, %s)",
  ...
  while (1) {
    ...
    if (timeout >= 0) {
      ...
      if (elapsed >= timeout) {
        ...
        if (g_state.use_syslog) {
          ...
          syslog(LOG_WARNING,
                 "timeout waiting for lock '%s' after %.1f seconds", descriptor,
                 timeout);
          closelog();
#endif
        }
        error(E_TIMEOUT, "Timeout waiting for lock '%s' after %.1f seconds",
              descriptor, timeout);
        return E_TIMEOUT;
      }
    }
    ...
    if (dir) {
      while ((entry = readdir(dir)) != NULL) {
        if (strncmp(entry->d_name, descriptor, strlen(descriptor)) == 0 &&
          ...
          safe_snprintf(check_path, sizeof(check_path), "%s/%s", lock_dir,
                        entry->d_name);
          if (read_lock_file_any_format(check_path, &existing_info) == 0) {
            if (existing_info.magic == LOCK_MAGIC &&
                validate_lock_checksum(&existing_info)) {
    ...
    if (active_locks >= max_holders) {
      if (timeout <= 0)
        return E_BUSY; // Fail fast if no timeout
      ...
      if (slot_claimed >= 0) {
        ...
        safe_snprintf(g_state.lock_path, sizeof(g_state.lock_path), "%s",
                      lock_path);
        return E_SUCCESS;
      }
    }
        ...
        if (dir) {
          ...
          while ((entry = readdir(dir)) != NULL) {
            if (strncmp(entry->d_name, descriptor, strlen(descriptor)) == 0 &&
              ...
              safe_snprintf(lock_file_path, sizeof(lock_file_path), "%s/%s",
          ...
          if (holder_pid > 0) {
            syslog(LOG_INFO, "lock '%s' held by PID %d", descriptor,
                   (int)holder_pid);
          } else {
            syslog(LOG_INFO, "lock contention for '%s' (waiting)", descriptor);
          }
        } else {
          syslog(LOG_INFO, "lock contention for '%s' (waiting)", descriptor);
        }
    ...
    if (timeout >= 0) {
      ...
      double elapsed_check =
          (now_check.tv_sec - start_time.tv_sec) +
      ...
      int max_sleep_ms =
          (int)(remaining * 1000 * TIMEOUT_FACTOR); /* 90% of remaining */
      if (max_sleep_ms < sleep_ms) {
        sleep_ms = max_sleep_ms;
      }
      if (sleep_ms < 1)
        sleep_ms = 1; /* Minimum 1ms */
    }
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (dot && strncmp(entry->d_name, descriptor, dot - entry->d_name) == 0 &&
      ...
      if (read_lock_file_any_format(check_path, &info) == 0) {
        if (info.magic == LOCK_MAGIC && validate_lock_checksum(&info) &&
          ...
          if (g_state.use_syslog) {
            ...
            openlog("waitlock", LOG_PID, g_state.syslog_facility);
            syslog(LOG_WARNING,
                   "removed corrupted lock file: %s (invalid checksum)",
                   entry->d_name);
            closelog();
#endif
          }
...
#ifdef HAVE_SYSLOG_H
    openlog("waitlock", LOG_PID, g_state.syslog_facility);
    syslog(LOG_INFO, "check lock '%s': %s (%d/%d holders)", descriptor,
           (active_locks >= max_holders) ? "busy" : "available", active_locks,
           max_holders);
    closelog();
#endif
  }
...
int list_locks(output_format_t format, bool show_all, bool stale_only) {
  ...
  dir = opendir(lock_dir);
  if (!dir) {
    error(E_SYSTEM, "Cannot open lock directory '%s': %s", lock_dir,
          strerror(errno));
    return E_SYSTEM;
  }
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (strstr(entry->d_name, ".lock")) {
      ...
      safe_snprintf(lock_path, sizeof(lock_path), "%s/%s", lock_dir,
                    entry->d_name);
      ...
      if (format == FMT_HUMAN) {
        if (is_stale) {
          printf("  [STALE]          (%-4d) %-4s %-8s %-19s %s\n",
                 (int)info.pid, info.lock_type == 1 ? "n/a" : "-", username,
                 time_str,
          ...
          if (info.lock_type == 1) {
            ...
            printf("%-18s %-6d %-4d %-8s %-19s %s\n", info.descriptor,
                   (int)info.pid, info.slot, username, time_str, info.cmdline);
          } else {
            /* Mutex - no slot */
            printf("%-18s %-6d %-4s %-8s %-19s %s\n", info.descriptor,
                   (int)info.pid, "-", username, time_str, info.cmdline);
          }
        }
      } else if (format == FMT_CSV) {
        printf("%s,%d,%d,%s,%ld,%s,%s\n", info.descriptor, (int)info.pid,
               info.slot, username, (long)info.acquired_at,
               is_stale ? "stale" : "active", info.cmdline);
      } else if (format == FMT_NULL) {
        printf("%s%c%d%c%d%c%s%c%ld%c%s%c%s%c%c", info.descriptor, '\0',
               (int)info.pid, '\0', info.slot, '\0', username, '\0',
               (long)info.acquired_at, '\0', is_stale ? "stale" : "active",
               '\0', info.cmdline, '\0', '\0');
      }
    }
  }
...
int done_lock(const char *descriptor) {
        ...
        if (validate_lock_checksum(&info)) {
          ...
          if (process_exists(info.pid)) {
            ...
            if (kill(info.pid, SIGTERM) == 0) {
              debug("Sent SIGTERM to process %d for lock %s", info.pid,
                    descriptor);
              released_locks++;
...
#ifdef HAVE_SYSLOG_H
                openlog("waitlock", LOG_PID, g_state.syslog_facility);
                syslog(LOG_INFO, "signaled process %d to release lock '%s'",
                       info.pid, descriptor);
                closelog();
#endif
              }
            } else {
              debug("Failed to send SIGTERM to process %d: %s", info.pid,
                    strerror(errno));

              /* If signal failed, check if process is actually gone */
              if (!process_exists(info.pid)) {
                debug("Process %d no longer exists, removing stale lock",
                      info.pid);
                unlink(lock_path);
                released_locks++;
              }
  ...
  if (released_locks == 0) {
    if (!g_state.quiet) {
      error(E_SYSTEM, "Failed to release any locks for descriptor '%s'",
            descriptor);
    }
    return E_SYSTEM;
  }
